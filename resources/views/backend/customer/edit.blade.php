        ...
        <div class="card">
            ...
            <div class="card-header">
                ...
                <div class="card-actions d-flex">
                    ...
                    <div class="p-1">
                        <button type="submit" id="save" title="Müşteri Güncelle" class="btn btn-primary">
            ...
            <div class="card mb-3">
                ...
                <div class="card-body">
                    ...
                    <div class="row mb-3">
                        ...
                        <div class="col-md-5">
                            <select name="city_id" id="citySelect" class="form-select">
                                ...
                                @foreach($cities as $city)
                                    <option value="{{ $city->plate_no }}" @selected($edit->city_id == $city->plate_no)>
                                        {{ $city->name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="col-md-4">
                            <select name="district_id" id="districtSelect" class="form-select" @if(!$edit->city_id) disabled @endif>
                                <option value="">İlçe Seçiniz</option>
                                @if($edit->district)
                                    <option value="{{ $edit->district->id }}" selected>
                                        {{ $edit->district->name }}
                                    </option>
                                @endif
                            </select>
                    ...
                    <div class="mb-3">
                        ...
                        <label class="form-check">
                            <input class="form-check-input" 
                                   ...
                                   name="status" 
                                   value="{{ $item->value }}" 
                                   @checked($edit->status?->value == $item->value)>
                            <span class="form-check-label">{{ $item->title() }}</span>
                        </label>
                        @endforeach
                    </div>
                    <hr>
                    <livewire:customer-tag-manager :model="$edit" />
                </div>
            </div>
        </div>
    </div>
...
<script>
    document.addEventListener('livewire:initialized', () => {
        Livewire.on('tags-updated', (data) => {
            document.querySelector('input[name="selectedTags"]').value = JSON.stringify(data.selectedTags || []);
        });
    });
</script>
@endpush