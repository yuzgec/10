...
{!! html()->model($edit)->form('PUT', route('translation.update', $edit->id))->open() !!}
    ...
    <div class="col-md-12">
        <div class="card">
            ...
            <div class="card-body">
                <x-dashboard.form.input label='Group Adı' name='group' placeholder="Group Adı Giriniz" disabled/>
                <x-dashboard.form.input label='Anahtar Adı' name='key' placeholder="Anahtar Adı Giriniz"/>

                @foreach ($locales as $locale)
                    <div class="mb-3">
                        <x-dashboard.form.input label='Çeviri [{{ $locale }}]' name="text[{{ $locale }}]" placeholder="Anahtar Adı Giriniz"/>
                    </div>
                @endforeach
